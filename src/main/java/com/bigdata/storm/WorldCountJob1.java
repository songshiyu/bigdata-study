...
public class WorldCountJob1 {
        ...
        public static void main(String[] args) throws Exception {
            if (args == null || args.length < 2){
                System.err.println("参数错误，参入的参数需要包含输入路径和输出路径！！");
                System.exit(-1);
            }
            ...
            String jobName = WorldCountJob1.class.getSimpleName();
             Job job =  Job.getInstance(conf,jobName);
            //添加我们要运行的主函数所在的类
            job.setJarByClass(WorldCountJob1.class);

            //TODO 设置mr的输入参数，设置计算的文件  指定 使用哪个mapper来进行计算
//            FileInputFormat.setInputPaths(job,"hdfs://input/mr/hello");
...
//            Path outPath = new Path("hdfs://ns1/output/mr/wc");       //如果该目录已经存在，会抛异常，目录存在异常
            outputPath.getFileSystem(conf).delete(outputPath,true);         //若目录存在，则递归删除该目录，即不会抛异常
